...
#if 0
	double lam2 = 
		(THCOND_d1 / ipow(Tbar,10) + THCOND_d2) * pow(rhobar,1.8) * 
...
#ifdef THCOND_DEBUG
	fprintf(stderr,"Q = %f, S = %f, pow(Tbar,10) = %f, lam2_1_1 = %f\n", Q, S, ipow(Tbar,10), lam2_1_1);
#endif	

	double lam2_1 = lam2_1_1 * pow(rhobar,9./5) * 
			exp(THCOND_C1 * (1. - pow(rhobar,14./5)));
