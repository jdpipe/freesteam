...
/**
	ASCEND external evaluation function
	Inputs: p,h
	Outputs: u,rho,T,x,s, mu
	@return 0 on success 
*/
int urhoTxsmu_ph_calc(struct BBoxInterp *slv_interp,
		int ninputs, int noutputs,
		double *inputs, double *outputs,
		double *jacobian
){
	...
	try{
		...
		CONSOLE_DEBUG("p = %f bar, h = %f kJ/kg --> u = %f kJ/kg, v = %f m^3/kg, x = %f"
			, double(p / bar), double(h / kJ_kg)
			, double(u / kJ_kg), double(rho / kg/m3)
			, x
		);
#endif
...
extern "C"{ // start of C-accessible portion
	...
	extern ASC_EXPORT int freesteam_register(){
		...
		result += CreateUserFunctionBlackBox("iapws97_urhoTxsmu_ph"
			, NULL /* alloc */
			, urhoTxsmu_ph_calc /* value */
			, NULL /* deriv */
			, NULL /* deriv2 */
			, NULL /* free */
			, 2,6 /* inputs, outputs */
			, "[u,rho,T,x,s,mu] = iapws97_uvTxsmu_ph(p,h) (see http://freesteam.sf.net)"
			, 0.0
		);
