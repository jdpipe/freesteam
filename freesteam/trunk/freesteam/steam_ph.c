...
SteamState freesteam_set_ph(double p, double h){
	...
	if(p <= p13){
		...
		S.R4.x = (h - hg)/(hf - hg);
		fprintf(stderr,"Region 4, T = %f, x = %f\n",S.R4.T, S.R4.x);
		return S;
	}

	fprintf(stderr,"HIGH PRESSURE REGIONS\n");
